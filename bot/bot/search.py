...
def search_item(item, res_size=CONST_ES_RESULT_SIZE, scroll_time='0s', advanced=False):
    ...
    try:
        if advanced is True:
            search = es.search(body={"query": {"query_string": {"default_field": "name", "query": item}}}, index=CONST_ES_ITEM_INDEX, size=res_size, scroll=scroll_time)                  # for an advanced search, let the user build the query string
        elif ' ' in item:
            search = es.search(body={"query": {"query_string": {"default_field": "name", "query": "\"" + item + "\""}}}, index=CONST_ES_ITEM_INDEX, size=res_size, scroll=scroll_time)    # for a full sentence, search with double quotes
        else:
            search = es.search(body={"query": {"query_string": {"default_field": "name", "query": "*" + item + "*"}}}, index=CONST_ES_ITEM_INDEX, size=res_size, scroll=scroll_time)      # for a single word, search with wildcards
