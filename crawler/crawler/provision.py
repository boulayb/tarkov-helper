...
def crawl_category():
    ...
    # get the wiki page
    logger.info("Getting HTML from " + CONST_BASE_URL + CONST_PROVISION_PAGE)
    r = requests.get(CONST_BASE_URL + CONST_PROVISION_PAGE)
    r.raise_for_status()
    ...
    # get the loot table
    loot_table = page_soup.find(id='List').parent.find_next('table', {'class': 'wikitable'}).find_all("tr")
